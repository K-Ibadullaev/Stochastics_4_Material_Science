...
plotALXFct <- function(BW, ALX, ALX2=NULL, show.type=0) {
  ...
  if(show.type %in% c(0,2)) {
    ...
    if(!is.null(ALX2)) lines(ALX2[,1], ALX2[,3], lwd=2, col=4)#legend 
...
# CORRECTED WINDOW SIZE 1->5 IN EACH C()!
rBM.rect.const <- function(lambda, a, b, W = owin(c(0,5),c(0,5))) {
...
digitizeRectSys <- function(M, spacing, xrange = attributes(M)$window$xrange, yrange = attributes(M)$window$yrange) {
  ...
  # CORRECTED xrange = attributes(M)$window$xrange, yrange = attributes(M)$window$yrange WITH M INSTEAD XYAP !!
  library(spatstat)
